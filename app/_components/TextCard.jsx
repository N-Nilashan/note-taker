...
const TextCard = ({ title, content, onDelete, onEdit, date, noteId, summary: savedSummary, category, isPinned, onPin, searchQuery }) => {
  ...
  return (
    <>
      <div
        className={`relative w-96 bg-[#F8F5F2] dark:bg-[#2D2D3A] shadow-lg border border-[#E5E0D9] dark:border-[#3D3D4A] rounded-xl p-6 transition-all duration-300 hover:shadow-xl ${isPinned ? 'border-2 border-emerald-500 dark:border-[#F8F5F2]' : ''
          }`}
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pt-4 mt-2 border-t border-[#E5E0D9] dark:border-[#3D3D4A]">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${loading || summarized
